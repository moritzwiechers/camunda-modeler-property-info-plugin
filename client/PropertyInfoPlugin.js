...
function PropertyInfoPlugin(eventBus, overlays, elementRegistry, editorActions) {
   ...
   function calculateSequenceFlowBadgeCoords(element, elementCount) {
      if(elementCount == null){
         elementCount = mainElementOverlays[element.id].length + 1;
      }
         ...
         switch (extensions[extension].$type) {
            case 'camunda:ExecutionListener':
               if (extensions[extension].event === 'start') {
                  ...
                  if(extensions[extension].expression &&  extensions[extension].expression.indexOf('execution.setVariable') > 0 && extensions[extension].expression.indexOf('TaskIstAsynchron') > 0 && extensions[extension].expression.indexOf('asyncForm') > 0) {
                     badges.push({
                        badgeKey: key + 'AS',
                        badgeSort: sort,
                        badgeType: 'AS',
                        badgeBackground: 'badge-orange',
                        badgeLocation: 'top-right'
                     });
                     key = '';
                  }
                  ...
                  if(extensions[extension].expression &&  extensions[extension].expression.indexOf('execution.setVariable') > 0 && extensions[extension].expression.indexOf('asyncForm') > 0 && extensions[extension].expression.indexOf('false') > 0) {
                     badges.push({
                        badgeKey: key + 'ASfalse',
                        badgeSort: sort,
                        badgeType: 'AS',
                        badgeBackground: 'badge-gray',
                     });
                     key = '';
                  }
                  if(extensions[extension].expression &&  extensions[extension].expression.indexOf('execution.setVariable') > 0 && extensions[extension].expression.indexOf('asyncForm') > 0 && extensions[extension].expression.indexOf('true') > 0) {
                     badges.push({
                        badgeKey: key + 'AStrue',
                        badgeSort: sort,
                        badgeType: 'AS',
   ...
   function addOverlays(badgeList, element) {
      ...
      for (var overlayCounter in sortedBadgeList) {
         ...
         } else if (overlayObject.badgeKey.indexOf('camunda:ExecutionListener-take') >= 0) {
            const badgeCoords = calculateSequenceFlowBadgeCoords(element, sequenceCounter);
            badges.push(overlays.add(element, 'badge', {
               position: {
